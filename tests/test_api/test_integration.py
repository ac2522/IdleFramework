...
UPLOAD_GAME = {
    ...
    "nodes": [
        {"id": "coins", "type": "resource", "name": "Coins", "initial_value": 100},
        {
            "id": "miner",
            "type": "generator",
            "name": "Miner",
    ...
    "edges": [
        {"id": "miner_to_coins", "source": "miner", "target": "coins", "edge_type": "production_target"},
    ],
    "stacking_groups": {},
}
...
class TestPlaySessionFlow:
    """End-to-end: start -> advance -> purchase -> advance -> auto-optimize -> delete."""
    ...
    def test_play_session_flow(self, client):
        ...
        assert resp.status_code == 200
        data = resp.json()
        assert data["elapsed_time"] == pytest.approx(30.0, abs=0.5)
        # Resources should have grown
        first_resource = list(data["resources"].values())[0]
        ...
        assert resp.status_code == 200
        assert resp.json()["generators"][gen_id]["owned"] >= 1

        # 4. Advance more time
        resp = client.post(f"/api/v1/engine/{session_id}/advance", json={"seconds": 10.0})
        assert resp.status_code == 200
...
class TestAnalysisThenReport:
    """End-to-end: run analysis -> generate HTML report."""
    ...
    def test_analysis_then_report(self, client):
        ...
        # 2. Generate HTML report for the same game
...
class TestGameUploadAndAnalyze:
    """End-to-end: upload game -> analyze it -> delete it."""

    def test_game_upload_and_analyze(self, client):
        # 1. Upload a new game
        resp = client.post("/api/v1/games/", json=UPLOAD_GAME)
